...
<%--  include --%>
	...
	<div class="container mt-5">
		<h5>專欄類型</h5>
	           ...
	           <tbody>
	           <c:forEach var="columnType" items="${listAll}">
	               <tr>
	                   <td>${columnType.colTypeId}</td>
	                   <td contenteditable="true">${columnType.colTypeName}</td>
  ...
  $(document).ready(function() {
    ...
    $('td[contenteditable="true"]').on('blur', function() {
        ...
        var colTypeName = currentRow.find('td:eq(1)').text().trim();
    	if(colTypeName === ''){
    		Swal.fire({
    			icon: 'error',
    			title: '專欄類型名稱不能為空!!!'
        ...
        $.ajax({
        	url:'<%=request.getContextPath()%>/columntypesmg/edit',
        	type:'POST',
            ...
            $.ajax({
            	...
            	success:function(response){
            		if(response.status === "success") {
                        ...
                        currentRow.find('td:eq(0)').text(response.newColumnTypeId);
            		}
            	},
            	error: function(xhr, status, error) {
            		console.error('輸入失敗', xhr, status, error);
